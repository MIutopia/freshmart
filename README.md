...
当前为 `v0.1.0` 的架构与规则实现阶段，已完成数据库迁移、核心金额与仓配规则类、接口草案和前端壳；业务接口、认证、真实支付、地图和大模型供应商仍未接入。
...
- 每个可交付阶段创建 Git commit；功能在 `feature/<module>-<topic>` 分支开发，审核后才合并 `main`。远程仓库为 `https://github.com/MIutopia/freshmart`。
