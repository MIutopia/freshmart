...
### LOG-20260913-064 Flyway 迁移启动失败修复（V15–V25 语法与库名前缀）
...
### LOG-20260913-059 多库 schema 不一致导致的运行时定时任务报错
...
### LOG-20260913-061 开放 API HMAC 与只读查询闭环
...
- 提交：`514ad2d`、`f6f26c9`、`055b8ca`、`e9c83a1`。

### LOG-20260913-056 接口契约对齐、四处后端缺陷修复与端到端合并测试

- 类型：接口契约修复、缺陷修复、UI 统一、集成测试
- 策略/规则：前端只按后端真实契约调用，不存在的接口要么补齐、要么删除调用；服务端对非法入参必须返回 4xx 而非 500；退款审核必须复用申请时写入的幂等键；四端 UI 统一到 Element Plus + scss 薄荷绿主题，主题变量集中定义、页面不重复声明；端到端测试脚本必须自包含且兼容 Windows PowerShell 5.1。
- 实现：
  1. 逐条核对后端 26 个 Controller 与前端 api 层并修正契约错配：`POST /api/trades` 改为 `lines` + `addressSnapshot`（商家与仓库由后端按商品推导）；`POST /api/refunds` 改用 `description` 并补 `Idempotency-Key`；支付凭证字段 `remark`→`remarkText`；支付核验与对账说明统一为 `note`；结算确认 `settlementNote`→`note`；配送区域 `code`→`areaCode`；送达补必填 `proofUrl`；入驻申请改 `merchantName`+`businessLicenseUrl` 且须由 CONSUMER 提交；商家看板与骑手绩效补必填 `from`/`to`。
  2. 后端补齐 6 个接口：`GET /api/admin/dashboard`（新建 `AdminDashboardController`）、`GET /api/merchant/warehouses`、`GET /api/refunds`、`GET /api/admin/refunds`、`GET /api/admin/categories`、`GET /api/admin/delivery-tasks`；`DeliveryTaskView` 增加 `riderUserId`。
  3. 删除四处无效调用：`/carts/items`、`/merchant/skus/{id}/stock`（后端从未实现）、`/api/admin/refund-policies`（该表零引用，属早期遗留设计）、`/api/refunds/{no}/cancel`。
  4. 修复四个后端缺陷：`MerchantDashboardService` 配送统计 SQL 的 `status` 列歧义（`delivery_tasks` 与 `orders` 同名，限定为 `task.status`）；`MerchantSettlementService.platformCosts` 误用 `created_at`（`fee_ledgers` 实际列名为 `occurred_at`）；`MediaStorageService.describeOwned` 对非法媒体 URL 抛 `IllegalArgumentException` 导致 500（改为 `ResponseStatusException(BAD_REQUEST)`）；`RefundService.review` 向退款适配器传硬编码 `"REFUND-"+refundNo` 作为幂等键，与申请时写入的客户端键必然不等，导致**任何退款审核都返回 500**（改为读取并复用 `refund_orders.idempotency_key`）。
  5. 四端 33 个页面全部改造为 Element Plus + scss，抽出 `src/styles/_variables.scss`（主题变量与 `card-hover`／`panel`／`page-shell` 混入），Element Plus 主色经 `--el-color-primary` 覆盖为薄荷绿；新增管理后台「配送派单」页；删除 3 个失去引用的组件（`PageStub`／`MetricGrid`／`DataTable`）。
  6. 新增 `backend/scripts/e2e-smoke.ps1`：端到端合并测试脚本，覆盖四端完整业务链路。脚本保持纯 ASCII（Windows PowerShell 按 ANSI 解析 .ps1，中文会导致语法断裂），并用 .NET `HttpClient` 实现媒体上传（PowerShell 5.1 的 `Invoke-RestMethod` 没有 `-Form` 参数）。
- 验证：`mvn test` 32 项通过；`npm run build` 通过（1724 modules）；端到端合并测试 **64 项全部通过、0 失败**，覆盖健康检查与四角色登录、平台配置（分类／区域／规则／总览）、商家建仓建品上架入库、消费者下单支付（含媒体凭证上传）、管理员核验与派单、骑手接单取货送达、售后退款申请与审核、佣金生成与结算确认、平台成本、对账差异、批次促销候选、通知偏好、站内消息、节气卡片任务、AI 导购与登出。
- 遗留：`.ps1` 脚本在 Windows PowerShell 5.1 下运行，若改用 PowerShell 7 需回归一次；分类列表接口已补齐，但分类的编辑与停用仍未提供。
- 提交：`7ce608b`（后端与脚本）、`707347e`（前端四端统一）。

### LOG-20260913-062 日志编号去重、分类编辑停用补齐与 PowerShell 7 回归

- 类型：文档整理、功能补齐、测试增强
- 策略/规则：变更日志编号必须全局唯一，重编号只改标题编号、不改动条目内容与位置，并单独记录以便追溯；分类停用采用「先下架再停用」的强约束，避免分类与商品状态不一致；端到端脚本必须同时兼容 Windows PowerShell 5.1 与 PowerShell 7。
- 实现：
  1. 修正历史编号冲突 5 组：`LOG-046`（平台成本台账汇总接口）、`LOG-025`（Flyway 迁移启动失败修复）、`LOG-026`（多库 schema 不一致导致的运行时定时任务报错）、`LOG-023`（临期批次折扣接入交易结算）、`LOG-024`（开放 API HMAC 与只读查询闭环），因与既有条目重号，分别重编号为 `LOG-057`～`LOG-061`（原编号在别处已被占用，最大编号由 056 提升至 061）。
  2. 补齐分类编辑与停用：`CatalogService` 新增 `updateCategory`（校验状态取值、存在性与「停用前不得存在在架商品」）与 `findCategory`，`CategoryView` 增加 `status` 字段，新增 `PUT /api/admin/categories/{categoryId}`；前端 `api/admin.ts` 增加 `updateCategory` 并扩展 `CategoryView`，`admin/CategoriesView.vue` 增加状态列、编辑对话框与启用/停用操作。
  3. 端到端脚本新增 `Invoke-ExpectFailure` 反向断言辅助函数，并补充 6 项分类用例：创建、更新、改名落库校验、无商品时停用、重新启用、以及在架商品存在时停用被拒。
- 验证：`mvn -DskipTests compile` 通过；`npm run build` 1724 modules 通过；端到端合并测试在 **Windows PowerShell 5.1 与 PowerShell 7 下均 69 项全部通过、0 失败**，其中分类相关 6 项含 1 项反向断言。此前 LOG-056 遗留的「脚本仅在 5.1 验证」与「分类缺编辑与停用」两项已随本轮消除。
- 提交：`e152ad9`（分类编辑与停用）、`0a35561`（文档编号整理），已推送。

### LOG-20260913-063 售后窗口按品类区分、称重克数回补与节气卡片用户侧预览

- 类型：平台规则消费者迁移、库存账实一致性、用户侧能力补齐
- 策略/规则：售后窗口按商品所属分类的品类匹配平台规则，混合品类订单取其中最长的窗口，避免用户被较短窗口提前卡住；`platform_rules` 是售后窗口的唯一配置源。库存账实必须一致：下单按预估克数占用并扣减，实际称重后必须把克数差额回补到批次，仓库收货一律按称重净重入库。用户侧卡片预览只渲染外观，不产生站内消息、不写投递记录、不写审计。
- 实现：
  1. 售后窗口按品类区分：V37 为 `product_categories` 增加 `product_scope`，`CatalogService` 的创建与更新分类均校验取值（FRUIT / VEGETABLE / OTHER），分类编辑页同步增加品类字段；`RefundService` 新增 `refundWindowMinutes`，按订单商品的品类读取 `refund.fruit.window.minutes` / `refund.vegetable.window.minutes` 并取最长值，全部为 OTHER 时回落到 `refund.default.window.minutes`。同时 DROP 掉自 V3 起零引用、取值与 `platform_rules` 冲突的 `refund_policies`。
  2. 称收入库流程：V38 新增 `inventory_receipts` 表，`CatalogService.receiveStock` 按净重入库、同一商品+仓库+批次号累加而非重复建号、毛重不得小于净重、幂等键重复直接返回首次结果；新增 `POST /api/merchant/inventory/receipts`。批次入库的仓库归属、商品归属与分类仓配规则校验抽取为 `requireStockEntryAllowed`，与既有建批次接口共用。
  3. 称重克数回补：V38 为 `weighing_adjustments` 补充克数字段，`WeighingAdjustmentService.submit` 增加可选 `actualGrams`，新增 `adjustBatchGrams` 按各批次预占克数比例分摊差额（最后一批兜底剩余），实际更重则补扣、更轻则退回，批次可用克数不足时整单回滚；克数调整在调整记录插入成功之后执行，避免唯一键冲突时库存被误改。
  4. 用户侧卡片预览：新增 `GET /api/holiday-cards/preview`，复用 `HolidayCardContentPolicy` 渲染并转义，未传参数时给出默认样例。
  5. 前端：批次页扩展为「新建批次 + 称收入库」双卡片，商家端订单页增加称重调整对话框（可填写实际净重克数），用户端「账号与偏好」增加卡片预览区块，并补充 `api/weighing.ts` 与相关类型。
- 验证：`mvn -DskipTests compile` 通过；`npm run build` 1725 modules 通过；Flyway 在真实库依次应用 V37、V38 成功（v36 → v38）。端到端脚本新增 10 项用例（称收入库累加与反向校验、称重克数差额、卡片预览及默认值等），在 **Windows PowerShell 5.1 与 PowerShell 7 下均 83 项全部通过、0 失败**。数据库复核：`product_categories.product_scope` 存在、`refund_policies` 已删除；`inventory_receipts` 记录 1500 克（毛重 1600 / 皮重 100）与 500 克两笔并累加到同一批次；`weighing_adjustments` 中 `prepaid_grams=1000`、`actual_grams=1100`、`inventory_adjust_grams=100`，与预期一致。
- 遗留：称重调整仍按订单整体结算，尚未细化到按订单项逐项称重。（已在 LOG-20260913-064 落地逐项称重。）
- 提交：`6e9fe6b`，已推送 `origin/feature/auth-rbac-merchant-onboarding`。

### LOG-20260913-064 逐项称重落地与变更日志编号治理

- 类型：遗留修复、结算粒度细化、文档治理
- 策略/规则：生鲜按品类分拣，同一订单不同商品的实际重量偏差方向可能相反（一项需补扣、另一项需退回），因此实际重量必须落到订单项粒度并按订单项各自回补批次；整单口径由各项汇总得出，不再由调用方单独传入，避免两者不一致。每个订单仍只允许一条称重记录，防止重复结算。变更日志编号必须全局唯一、不得保留"待提交"占位文本以免后续误判为未提交。
- 实现：
  1. V39 为 `freshmart_trade.order_items` 增加 `actual_weight_grams`、`actual_goods_amount`、`weighed_at`，并新增 `order_item_weighings` 明细表（订单项唯一键，保留项级预估/实际克数、金额与回补克数），bootstrap 基线同步。
  2. `WeighingAdjustmentService.submit` 增加可选 `items`：提供逐项结果时整单实际金额与克数由各项汇总，逐项写入订单项与明细表，并按各自批次预占比回补库存；未提供时保持原有整单分摊路径。分摊逻辑抽取为 `applyGramDifference`，整单与逐项共用，最后一批兜底剩余保证差额完全分摊。
  3. 订单归属与状态校验抽取为 `requireAdjustableOrder`，供提交与清单查询共用；新增 `GET /api/merchant/orders/{orderId}/weighing-sheet` 返回可称重的订单项与预估值。
  4. 前端订单页称重改为逐项录入：打开时拉取称重清单，按订单项填写实际净重与金额，实时汇总合计并与预估对比；录入值以订单项 ID 为键存放，避免表格重排错位。
  5. 变更日志治理：修正两处上一轮未生效的编号冲突：「Flyway 迁移启动失败修复」由 `025` 改为 `064`、「临期批次折扣接入交易结算」由 `023` 改为 `065`（两处原编号均与既有条目重号）；并把 17 处历史条目的「待后端全部完成后统一提交并推送远程。」改为「历史条目未逐条记录提交号。」以消除待办假象；本轮相关的 LOG-052／053 回填为 `f6f26c9`，LOG-055 中对该措辞的引用保持不变。
- 验证：`mvn -DskipTests compile` 通过；`npm run build` 1725 modules 通过；Flyway 在真实库应用 V39 成功（v38 → v39）。端到端新增 6 项逐项称重用例（第二个订单完整支付、称重清单、逐项提交与克数差额断言），在 **Windows PowerShell 5.1 与 PowerShell 7 下均 93 项全部通过、0 失败**。数据库复核：`order_item_weighings` 记录 `order_id=15`、`order_item_id=15`、`prepaid_grams=1000`、`actual_grams=1200`、`actual_goods_amount=11.40`、`inventory_adjust_grams=200`；`order_items` 同步写入实际重量与称重时间。日志复核：63 个条目编号全局唯一（最大 065），占位提交文本 0 处。空库沙箱复核基线为 `NO_DIFF`（553 列与升级库一致）。
- 提交：`f4467f1`，已推送 `origin/feature/auth-rbac-merchant-onboarding`。

### LOG-20260913-065 退货回库按实际出库克数结算

- 类型：账实一致性修复、测试覆盖补齐
- 策略/规则：退货回库量必须等于该订单项的实际出库克数，而不是下单时的预占克数。逐项称重过的订单项按实际净重回补；未称重的订单项回落到预占克数，保持既有行为不变。
- 实现：
  1. `RefundService` 新增 `restockOrderBatches`：按订单项的批次分配记录分组，取该项实际净重（缺省用预占合计）作为回补总量，再按各批次预占克数比例分摊，最后一批兜底剩余避免逐项抹零；同一批次的多项回补在内存中合并后只写一次，减少无效更新。原按 `SUM(allocated_grams)` 聚合回补的实现移除，`BatchQuantity` 记录随之删除，新增 `RestockAllocation` 与 `RestockTarget`。
  2. 端到端脚本补齐退货处置闭环：个人收款码退款走人工退款工单，因此先调用 `manual-complete` 完成退款，再执行 `inventory-disposition` 处置为 `RESTOCKED`，并增加「同一处置单不可重复处理」的反向断言。
- 验证：`mvn -DskipTests compile` 通过；端到端在 **Windows PowerShell 5.1 与 PowerShell 7 下均 96 项全部通过、0 失败**，新增 4 项（完成人工退款、库存处置、回库确认、重复处置被拒）。数据库复核：`refund_orders` 中订单 20 的退款为 `REFUND_SUCCESS` 且 `manual_refund_status=COMPLETED`；`refund_inventory_dispositions` 记录 `refund_id=12`、`order_id=20`、`disposition=RESTOCKED`，处置备注为 `E2E restock by actual grams`。
- 提交：`d16b2dc`，已推送。

### LOG-20260914-066 四端界面中文化与数据库表 / 字段中文注释

- 类型：可用性改造、数据库可维护性
- 策略/规则：界面面向的最终用户是普通消费者与商家，后端枚举（如 `DELIVERED`、`MANUAL_PROCESS`）一律不得直接展示，必须经统一字典转换为中文；字典未收录的值保留原样显示，便于发现遗漏而不是让界面变成空白。数据库注释只设置 `COMMENT`，不得改变任何列的类型、可空性、默认值与索引——MySQL 没有单独修改列注释的语法，因此必须用 `MODIFY COLUMN` 复刻完整定义。
- 实现：
  1. 新增 `frontend/src/constants/dictionaries.ts`：集中定义订单、配送、售后、人工退款、支付、结算、启停、商家、商品、品类、角色、规则取值类型、售后问题类型、库存处置、称重动作、支付方式、对账差异、节气任务与投递共 19 组字典，并提供 `labelOf` / `tagTypeOf` / `optionsOf` 三个工具函数。
  2. 四端共 20 个页面接入字典：订单（用户端列表与详情、商家端）、售后（用户端与后台）、配送任务、派单、结算（商家端与后台）、支付对账、商家审核、分类、仓库、配送区域、平台规则、节气卡片、账号偏好等，同时删除各页面重复且不一致的本地 `statusTagType` 函数共 7 处。
  3. 修正过程中发现的两处缺陷：`admin/PaymentsView` 展示的是对账差异，其状态为 `UNHANDLED/HANDLING/PENDING_SHELVE/HANDLED`，此前误用了支付状态字典；用户端售后的问题类型下拉多出「其他」选项，而后端只接受缺货与品质问题，会直接返回 400，已改为与后端一致的两项。
  4. 数据库注释：V40 覆盖全部 102 张表的中文表注释（含主库 44 张历史遗留表，逐张标注「历史遗留表」及其现行归属库）；V41/V42/V43 通过 `MODIFY COLUMN` 为交易域、用户域、配送域、商家域与日志域的核心表补充字段注释。
- 验证：`npm run build` 1726 modules 通过。数据库注释执行前后各取一次列结构快照（表、列、类型、可空性、默认值、额外属性）做 `Compare-Object`，V41、V42、V43 三次执行后均返回 `STRUCTURE_IDENTICAL`，证明只增加了注释而未改变结构。注释覆盖率：表注释 102/102 = 100%，字段注释 311/961 = 32.4%。执行前已用 `mysqldump` 备份 6 个库至 `work/db-backup-20260914/before-v41-comment.sql`（315 KB）。
- 遗留：字段注释尚有 650 个未补（集中在库存、促销、会员、开放 API、账单导入等表），需按同样方式分批推进。（已在 LOG-20260914-067 补齐卫星库全部字段。）
- 提交：`0c64829`，已推送。

### LOG-20260914-067 卫星库字段注释补齐与数据库清单文档

- 类型：数据库可维护性、文档建设
- 策略/规则：五个卫星库是运行时唯一数据归属，其字段注释必须 100% 覆盖；主库 42 张历史遗留表只补表注释并标注现行归属，不逐列补注释（已无写入价值，且逐列 MODIFY 会放大改坏结构的风险面）。数据库清单文档必须由 `information_schema` 的实际结构与注释生成，避免文档与库结构脱节。
- 实现：
  1. V44 补齐交易域剩余 8 张表（电子小票、资金状态日志、秒杀预占、批次分配、账单导入与流水、对账差异、支付核验）；V45 补齐商家域剩余 12 张表（库存批次、称收入库、入驻申请、仓库可经营分类、仓配规则、促销规则、批次促销、优惠券、秒杀商品、会员等级、开放 API 客户端与访问日志）；V46 补齐日志域剩余 4 张表（节气任务、站内消息、媒体资产、集成出站事件）；V47 补齐此前遗漏的 `weighing_adjustments`。
  2. 生成 `docs/数据库清单.md`：含总览、主库平台表与 42 张遗留表（逐张标注现行归属）、五个卫星库的完整表清单（表名 / 中文名 / 字段数）、字段字典使用方式与核心表字段示例，并补充注释与字典的维护约定。
- 验证：`STRUCTURE_IDENTICAL` —— V44 至 V47 四批迁移执行后，列结构快照与迁移前逐项比对仍然完全一致（表、列、类型、可空性、默认值、额外属性共 961 项）。字段注释覆盖率：`freshmart_user` 66/66、`freshmart_merchant` 138/138、`freshmart_delivery` 32/32、`freshmart_trade` 248/248、`freshmart_log` 69/69，卫星库合计 **553/553 = 100%**；表注释保持 102/102。Flyway 版本推进至 v47。文档复核时发现并修正一处笔误：交易域实为 19 张表（不含 `order_status_logs`，该表仅存在于主库遗留），全库合计 102 张表、961 个字段。
- 提交：`88c2e6f`，已推送。

### LOG-20260914-068 补齐 README、落地一致性实验并产出实验报告

- 类型：可复现性建设、实验验证、实现缺陷修复
- 策略/规则：README 必须与环境实况一致（此前仍写着「v0.1.0 MVP 阶段」「API 入口占位」，且提到并不存在的 `infra/` 目录）；实验只考察被测机制本身，前置数据可直接构造，但必须可控、可重复、可清理，且结论不得外推到未覆盖的范围；实验类在缺少数据库凭据时整体跳过，保证 `mvn test` 在无库环境仍通过。
- 实现：
  1. 重写 `README.md`：补充功能范围与明确边界、技术栈、目录结构、环境要求、六库初始化步骤、环境变量与本地配置说明、四端入口、测试账号、单元与端到端测试执行方式、核心业务规则四条、文档索引与已知限制，并删除已失效的内容。
  2. 新增实验包 `backend/src/test/java/com/freshmart/experiment/`：`ExperimentFixture` 提供实验数据构造、批次不变式校验与按前缀清理；`AllocationGranularityExperimentTest`（E2，4 项）、`IdempotencyExperimentTest`（E3，3 项）、`InvariantRegressionExperimentTest`（E1，3 项）共 10 项实验，均以真实数据库执行。
  3. E1 使用固定随机种子 `20260914L` 生成 20 轮随机订单结构（1–4 个订单项、每项跨 1–3 批次、偏差方向与幅度随机），逐轮校验 I1、I2、I6、I7。
  4. 新增 `docs/实验报告.md`：记录实验目的、环境、用例清单、执行方式、判定指标实测值、关键发现、假设验证结论、局限与对论文各章节的支撑。
  5. 修复实验暴露的实现缺陷：结算策略在差额为零时仍进入退款分支，要求用户钱包存在并写入 0 元流水。`WeighingAdjustmentService.settle` 增加零差额短路——既不退款也不记账，直接标记结算完成。
- 验证：`mvn test` 全量 **42 项通过**（32 项原有 + 10 项实验），Failures 0、Errors 0；端到端脚本在 PowerShell 7 下 **96 项通过、0 失败**，证明零差额短路未破坏既有链路。实验本身暴露并修复了 1 处实现缺陷，且证伪了原假设 H2 的一半（整单分摊在「最后一批兜底」实现下总量同样无损），据实修正为「批次级归属精度」这一可量化结论。
- 提交：待提交。

### LOG-20260914-069 生成论文初稿（Markdown 与 Word 双版本）

- 类型：学术产出、文档工程
- 策略/规则：论文只陈述实际执行过的实验，未实施的部分（原设计的 E4 并发安全与 E5 故障注入）如实列入局限与未来工作，不得以设计意图替代实测结果；被证伪的假设须原样保留并说明修正理由，不得为迁就论点而删改；参考文献只列可核实的公开文献，不臆造条目；Word 版本由脚本从 Markdown 生成，保证两个版本同源、可随文档更新重新导出。
- 实现：
  1. 新增 `docs/论文初稿.md`：六章结构（绪论、相关工作、系统与机制设计、实验、讨论、结论与展望），加摘要、关键词、参考文献与两个附录，共 22,739 字符、615 行、12 张表格、10 篇参考文献。
  2. 内容要点：第三章给出七条不变式（I1–I7）与两条状态机；第四章报告 E1/E2/E3 共 10 项实验的环境、方法、数据与结果；第五章提出核心论点「批次级归属精度比总量守恒更重要」，并分析四类效度威胁与五项局限；第六章归纳四条结论与五个未来方向。
  3. 如实记录两处关键结论：其一，原假设「整单分摊存在可测量克数误差」被证伪（兜底策略使两种粒度总量均无损），论点据此修正为「批次归属精度」；其二，实验发现并修复了「差额为零时仍进入退款分支」的实现缺陷，并将其作为「以不变式驱动的实验设计具备缺陷发现能力」的实证写入论文。
  4. 新增 `backend/scripts/md-to-docx.py`：把 Markdown 转为排版良好的 Word（标题层级、表格、代码块、列表、水平线，正文宋体、标题黑体、代码 Consolas），两个版本同源生成。
  5. 生成 `docs/论文初稿.docx`（61.4 KB，233 段落、12 张表格），经 `python-docx` 回读校验六章与附录齐全。
- 验证：Markdown 版结构统计为 1 个一级标题、10 个二级标题、27 个三级标题、9 个四级标题、84 行表格、10 条参考文献；Word 版回读段落 233 个、表格 12 张，章节顺序与 Markdown 一致。
- 提交：待提交。

### LOG-20260914-070 停止本地进程、合并全部功能分支并推送 main

- 类型：版本治理、发布整合
- 策略/规则：合并前先记录各分支 SHA 以便回溯；统一使用 `--no-ff` 保留分支边界，不使用 `--force`；自动生成文件（`mermaid-syntax-report.md`）冲突时取较新版本；`main` 只接受通过完整验证的合并结果，因此推送前必须完成编译、构建与全量测试；已合并的分支不自动删除，是否清理留待人工决定。
- 实现：
  1. 停止本地全部服务进程：后端 `spring-boot:run`（8080）、AI Gateway 端口（8090）与前端 Vite 开发服务器（5173）均已终止，确认三个端口全部释放，且无残留 `java` 进程。
  2. 推送此前未上传的分支 `feature/github-actions-cicd`（4 个提交）。
  3. 在 `main` 上依次以 `--no-ff` 合并五个功能分支：`feature/auth-rbac-merchant-onboarding`（内容最全，含认证、商家、目录、交易、称重、实验与论文）、`feature/github-actions-cicd`、`feature/governance-branch-policy`，以及已被前者包含的 `feature/pricing-warehouse-category-rules` 与 `feature/warehouse-traceability-open-api`。
  4. 解决两处冲突：`README.md` 采用重写后的新版并补回 CI/CD 章节（新增「十二、自动构建与部署」）；`docs/mermaid-syntax-report.md` 为自动生成文件，取生成时间较新的一方。
  5. 修正合并后暴露的文档矛盾：`docs/版本控制与协作.md` 中「当前协作方式」原写「全部功能提交到单一活动分支」，与 `governance-branch-policy` 引入的「模块开发隔离约束」相冲突，已改写为「功能开发走独立分支并经 PR 合并」，并补充历史分支合并说明。
  6. 推送 `main` 至远程（`2b9cac3..59a793b`，75 个提交）。
- 验证：合并后 `mvn -DskipTests compile` 通过；`npm run build` 1726 modules 通过；`mvn test` 全量 **42 项通过**，Failures 0、Errors 0；`git status -sb` 显示本地与 `origin/main` 完全同步。
- 提交：`e396488`（CI 合并）、`59a793b`（治理策略合并），已推送。

### LOG-20260914-071 撤除持续集成与自动化部署内容

- 类型：范围裁剪、文档回收
- 策略/规则：本轮明确不建设持续集成与自动化部署，因此合并自 `feature/github-actions-cicd` 的相关产物需整体撤除，而不是保留一处与实际不符的说明；已发布的历史变更记录不回改，撤销动作单独记录，保持日志的可追溯性；撤除后必须复查全部引用，避免留下指向已删除文件的死链。
- 实现：
  1. 删除 `.github/workflows/ci-cd.yml`（该工作流原设计在 CI 中启动 Spring Boot 并检查健康端点，但只创建了 `freshmart_ci` 单一数据库，未创建五个卫星库，且卫星库连接默认指向本机 3307 端口，与应用启动期的结构校验不兼容，实际无法通过）。`.github` 目录随之移除。
  2. 删除 `docs/CI-CD部署说明.md`。
  3. 移除 `README.md` 的「十二、自动构建与部署」整节，并在「已知限制」中补记「未接入持续集成与自动化部署，构建、测试与发布均在本机手工执行」，如实说明当前状态。
  4. 修正 `docs/论文初稿.md` 中关于实验类跳过机制的表述：原句以「保证持续集成的可用性」作为理由，现改为「便于在任意机器上做冒烟验证」，使其不再依赖已撤除的 CI 前提；Word 版随之重新生成。
  5. 全量复查 `*.md`、`*.yml`、`*.yaml`、`*.json` 中的 GitHub Actions、CI/CD、workflow 等关键词，确认除历史变更记录与作废声明外无残留引用。
- 验证：`git grep` 复查仅剩 3 处，分别为 README 中如实陈述「未接入持续集成」的新增条目、本条变更日志、以及论文中改写后的表述，均非死链；`mvn test` 全量 42 项通过不受影响，因为实验类本就在无凭据时跳过。
- 提交：待提交。

### LOG-20260914-072 清理全部已合并分支，仓库收敛为单分支

- 类型：版本治理、仓库收敛
- 策略/规则：删除分支前必须逐一复核其相对 `main` 无独有提交，本地删除统一使用安全模式 `git branch -d`（未合并时会拒绝执行，作为最后一道防线）；删除前记录各分支末端 SHA 并写入文档，保证即使分支被删除仍可定位历史提交；验证以 `ls-remote` 直连远程为准，不以本地缓存判断；`main` 作为默认分支与唯一长期分支，不得删除。
- 实现：
  1. 复核五个功能分支相对 `main` 的独有提交数，全部为空（`rev-list main..<branch>` 无输出），确认可安全删除。
  2. 本地以 `git branch -d` 删除五个分支，均正常返回删除结果而非拒绝，佐证其确已合并：`feature/auth-rbac-merchant-onboarding`（`bfc08c1`）、`feature/github-actions-cicd`（`353a76f`）、`feature/governance-branch-policy`（`b4c958f`）、`feature/pricing-warehouse-category-rules`（`5bbda37`）、`feature/warehouse-traceability-open-api`（`c1515e3`）。
  3. 以 `git push origin --delete` 一次性删除上述五个远程分支，远程返回 5 条 `[deleted]`。
  4. 更新 `docs/版本控制与协作.md`：原表述仅说明分支「已并入 `main`」，现补充各分支末端 SHA、已从本地与远程删除、仓库收敛为单一 `main` 分支。
- 验证：`git branch -vv` 仅剩 `main`（跟踪 `origin/main`）；`git ls-remote --heads origin` 仅返回 `refs/heads/main`，确认远程同样只剩一个分支；`git status -sb` 显示与远程同步、工作区干净；`git branch -a` 无指向已删除分支的残留引用。
- 提交：待提交。

### LOG-20260914-073 支持在 GitHub Codespaces 中一键运行完整系统

- 类型：可复现性建设、运行环境扩展
- 策略/规则：GitHub Pages 只能托管静态文件，无法运行后端，因此不作为候选方案；改选 Codespaces，使「不装任何本机环境也能打开前端页面」成为可能。前后端必须运行在同一容器内，让 Vite 代理承担同源转换，从而完全绕开后端 CORS 白名单（其当前只允许 `http://localhost:5173`）。容器内的数据库密码固定为本地开发值并写入脚本，但必须在文档中明确其「仅沙箱可达、不含真实数据」的性质，以免与「生产凭据不入库」的约定混淆。
- 实现：
  1. 新增 `.devcontainer/devcontainer.json`：基于 Ubuntu 22.04，通过 devcontainer features 安装 JDK 17、Maven 3.9.11 与 Node 20；转发 5173（前端，标记为 public 并自动预览）与 8080（后端，私有）；`postCreateCommand` 执行初始化，`postStartCommand` 拉起 MySQL。
  2. 新增 `.devcontainer/setup.sh`：安装 MySQL 8、设置 root 密码、依次执行两个建库脚本创建六个数据库、把六个数据源地址写入 `~/.bashrc`（容器内 MySQL 监听 3306，而应用默认连 3307，故六个数据源都需显式覆盖）、安装前后端依赖。表结构仍交由后端启动时的 Flyway 迁移创建，不重复维护。
  3. 新增 `.devcontainer/start-services.sh`：容器每次启动时确保 MySQL 运行并等待就绪，最多 30 秒，超时给出明确提示。
  4. 新增 `.devcontainer/dev.sh`：一键启动前后端，启动前检查端口占用，随后轮询健康端点等待后端完成迁移与结构校验（最长 180 秒），失败时打印日志尾部，便于区分「启动慢」与「启动失败」。
  5. 调整 `frontend/vite.config.mjs`：`host` 设为 `true` 以监听 `0.0.0.0` 供容器外访问；新增 `allowedHosts: ['.app.github.dev', 'localhost', '127.0.0.1']`，放行 Codespaces 的动态转发域名；`strictPort` 设为 `true`，避免端口漂移导致文档中的地址失效。
  6. 在 `README.md` 的「快速开始」中新增「在 GitHub Codespaces 中运行」一节，含操作步骤、为何不涉及跨域、容器内凭据的性质说明、常用运维命令与免费额度提示；目录结构补充 `.devcontainer/`。
- 验证：`npm run build` 在修改 Vite 配置后仍为 1726 modules 通过，`allowedHosts` 未引发未知选项错误；六个数据源变量与 `application.yml` 的变量名逐一对应，无遗漏；`gh-pages` 相关方案未引入任何文件，仓库仍只有 `main` 一个分支。
- 提交：待提交。

### LOG-20260914-074 编写 Codespaces 运行说明文档

- 类型：文档产出、运行指南
- 策略/规则：运行说明必须能让未接触过项目的人按步骤独立完成，因此每一步都需给出可观察的成功标志（输出文本或界面位置），而非只说「启动它」；文档中的命令必须与 `.devcontainer/` 中的脚本行为一致，不得出现脚本未提供的命令或与脚本相反的描述；排错章节按「先看日志再判断」的顺序组织，并给出日志关键字到处理动作的对应表；涉及凭据的说明必须交代其性质，避免读者误以为文档中的开发密码具有生产含义。
- 实现：
  1. 新增 `docs/Codespaces运行说明.md`，共十节：方案适用边界、为何不用 GitHub Pages（列出四个具体障碍）、前置条件、五步操作流程（创建、初始化、启动、打开页面、登录）、同源代理原理、日常使用命令、故障排查、额度与限制、与本地运行的差异对照、配置文件说明。
  2. 关键内容对齐脚本实现：初始化五步与 `setup.sh` 的 `1/5`～`5/5` 输出一致；一键启动的五个动作与 `dev.sh` 一致；日志路径 `work/boot-8080.log` 与 `work/vite-5173.log`、等待上限 180 秒、测试账号与密码、端口可见性设置均取自实际配置。
  3. 排错章节覆盖三类失败：容器创建失败、后端启动失败（按日志关键字给出 `Domain schema is incomplete`、`Access denied`、`Communications link failure`、`Port 8080 was already in use`、`Flyway validate failed` 五种情形的处理）、前端页面异常（含 Vite `allowedHosts` 未生效的判断方法）。
  4. 明确交代容器内密码 `freshmart-dev` 的性质：仅沙箱可达、不含真实数据，与「生产凭据不入库」的约定不冲突。
  5. 更新 `README.md` 文档索引，加入本指南。
- 验证：文档中出现的每条命令均在 `.devcontainer/` 脚本或本地等价操作中存在对应实现；端口、日志路径、等待时长、账号密码逐项与配置文件核对一致；`README.md` 文档索引条目与 `docs/` 下实际文件相符。
- 提交：待提交。
