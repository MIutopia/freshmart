# 系统架构设计

## 总体架构

系统采用前后端分离和模块化单体起步，保留后续拆分为独立服务的边界。Vue 前端按角色加载菜单与路由；Spring Boot 负责认证、业务编排、交易一致性和审计；AI Gateway 作为受控适配层，隔离模型供应商密钥与提示词。

```mermaid
flowchart LR
    U[用户端 Vue] --> API[Spring Boot API]
    M[商家端 Vue] --> API
    D[配送端 Vue] --> API
    A[管理后台 Vue] --> API
    API --> AUTH[认证与权限]
    API --> BIZ[商品 订单 库存 履约]
    API --> PAY[支付适配器]
    API --> MSG[消息与通知]
    API --> AI[AI Gateway]
    BIZ --> DB[(MySQL)]
    BIZ --> CACHE[(Redis)]
    BIZ --> OSS[(对象存储)]
    AI --> MODEL[模型供应商]
```

## 四端边界

| 端 | 路由前缀 | 主要职责 |
| --- | --- | --- |
| 用户端 | `/app` | 浏览、购物、支付、订单、售后、AI 导购 |
| 商家端 | `/merchant` | 店铺、商品、库存、接单、经营分析 |
| 配送端 | `/delivery` | 接单、取货、路线、配送、签收、异常 |
| 管理后台 | `/admin` | 平台治理、权限、审核、运营、审计、数据看板 |

## 推荐后端包边界

```text
com.freshmart
├── auth             认证、角色、权限、令牌
├── user             用户、地址、会员
├── catalog          分类、SPU、SKU、图片
├── inventory        库存、批次、预占、预警
├── order            购物车、订单、状态流水
├── payment          支付、退款、对账适配
├── fulfillment      分拣、配送任务、轨迹、签收
├── marketing        优惠券、活动、推荐位
├── ai               AI Gateway、会话、工具白名单
└── audit             操作日志、风控事件
```

## AI 能力与边界

首期建议开放：商品问答、口味/预算推荐、订单进度查询、客服 FAQ、商家经营摘要。模型不得直接修改订单、库存、退款或权限；所有工具调用先经过用户身份、数据范围和参数校验，并记录请求、工具、结果摘要和耗时。涉及价格、库存和履约时，以业务服务实时结果为准。

## 验证记录

- 2026-09-08：Mermaid 严格解析校验通过，报告见 `docs/mermaid-syntax-report.md`，共 2 个图表，问题数为 0。
- 2026-09-08：Java 21 与 Git 2.54 可用；当前环境未安装 Maven，后端未执行 Maven 构建。
- 2026-09-08：前端依赖安装成功并生成 `package-lock.json`。在当前受限工作区执行 Vite 构建时，esbuild 访问工作区上层目录被拒绝，报错为 `Cannot read directory "../../../../..": Access is denied`；待在正常开发终端或调整该目录权限后重新执行 `npm run build`。
