...
后台以平台规则 `batch.near.expiry.days`（默认 3 天）筛选仍有可售库存、未过期批次进入低价促销候选清单。管理员确认后创建批次促销，促销必须在批次过期日前结束且同一批次排期不得重叠；促销仅作用于指定批次，不修改同商品其他仓库或其他批次价格。
...
开放 API 首期服务于已审核商家或平台合作方。管理员创建客户端时只展示一次明文密钥，数据库仅保存 `secret_hash`。当前本地首期适配先校验 `X-Api-Key`、`X-Api-Secret`、有效期、最小权限范围和商家边界，并写入访问日志；HMAC 签名头列入后续增强。
