...
public class AuthService {
    ...
    public LoginResult login(String loginName, String password, String sourceIp) {
        ...
        return new LoginResult(token, expiresAt, currentUser(null, user.userId(), user.loginName()));
    }
    ...
    public Optional<CurrentUser> authenticate(String token) {
        ...
        List<SessionUser> matched = jdbcTemplate.query("""
                SELECT s.id AS session_id, u.id AS user_id, u.login_name
                ...
                resultSet.getString("login_name")), AuthTokenCodec.hash(token));
        if (matched.isEmpty()) {
            return Optional.empty();
        }
        ...
        return Optional.of(currentUser(user.sessionId(), user.userId(), user.loginName()));
    }
    ...
    private Optional<LoginUser> findLoginUser(String loginName) {
        return jdbcTemplate.query("""
                SELECT id, login_name, password_hash, status
                ...
                resultSet.getString("status")), loginName)
                .stream().findFirst();
    }

    private CurrentUser currentUser(Long sessionId, Long userId, String loginName) {
        Set<String> roles = new LinkedHashSet<>(jdbcTemplate.queryForList(
                "SELECT role_code FROM user_role_assignments WHERE user_id = ?", String.class, userId));
        return new CurrentUser(userId, sessionId, loginName, Set.copyOf(roles));
    }

    public record LoginResult(String accessToken, LocalDateTime expiresAt, CurrentUser user) {
    }

    private record LoginUser(Long userId, String loginName, String passwordHash, String status) {
    }

    private record SessionUser(Long sessionId, Long userId, String loginName) {
    }
}
